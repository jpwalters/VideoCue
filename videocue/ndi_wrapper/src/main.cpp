...
PYBIND11_MODULE(NDIlib, m) {
  ...
  py::class_<NDIlib_video_frame_v2_t>(m, "VideoFrameV2")
      ...
      .def_property(
          "data",
          [](py::object self_obj) {
            auto& self = self_obj.cast<NDIlib_video_frame_v2_t&>();
            if (!self.p_data)
              return py::array();
            int r = self.yres;
            int c = self.xres;
            ...
            std::vector<size_t> shape = {(size_t)r, (size_t)c, (size_t)b2};
            std::vector<size_t> strides = {b1, b2, b3};
            // Keep the VideoFrameV2 object alive as long as the array exists
            return py::array(dtype, shape, strides, self.p_data, self_obj);
          },
          [](NDIlib_video_frame_v2_t &self, const py::array_t<uint8_t> &array) {
            auto info = array.request();
            if (info.ndim != 3) {
              throw std::runtime_error("VideoFrameV2.data requires 3D array [height, width, bytes_per_pixel], got " +
                                       std::to_string(info.ndim) + "D array");
            }
            if (info.shape[0] <= 0 || info.shape[1] <= 0) {
              throw std::runtime_error("Invalid frame dimensions: height and width must be > 0");
            }
  ...
  py::class_<NDIlib_audio_frame_v2_t>(m, "AudioFrameV2")
      ...
      .def_property(
          "data",
          [](py::object self_obj) {
            auto& self = self_obj.cast<NDIlib_audio_frame_v2_t&>();
            if (!self.p_data)
              return py::array();
            ...
            std::vector<size_t> shape = {row, col};
            std::vector<size_t> strides = {col * size, size};
            // Keep the AudioFrameV2 object alive as long as the array exists
            return py::array(dtype, shape, strides, self.p_data, self_obj);
          },
          [](NDIlib_audio_frame_v2_t &self, py::array_t<float> &array) {
            auto info = array.request();
            if (info.ndim != 2) {
              throw std::runtime_error("AudioFrameV2.data requires 2D array [channels, samples], got " +
                                       std::to_string(info.ndim) + "D array");
            }
  ...
  py::class_<NDIlib_audio_frame_v3_t>(m, "AudioFrameV3")
      ...
      .def_property(
          "data",
          [](py::object self_obj) {
            auto& self = self_obj.cast<NDIlib_audio_frame_v3_t&>();
            if (!self.p_data)
              return py::array();
            size_t col = self.no_samples;
            size_t row = self.no_channels;
            ...
            auto dtype = py::dtype(py::format_descriptor<uint8_t>::format());
            std::vector<size_t> shape = {row, col};
            std::vector<size_t> strides = {col * size * 4, size};
            // Keep the AudioFrameV3 object alive as long as the array exists
            return py::array(dtype, shape, strides, self.p_data, self_obj);
          },
          [](NDIlib_audio_frame_v3_t &self, py::array_t<uint8_t> &array) {
            auto info = array.request();
            if (info.ndim != 2) {
              throw std::runtime_error("AudioFrameV3.data requires 2D array [channels, samples], got " +
                                       std::to_string(info.ndim) + "D array");
            }
            self.p_data = static_cast<uint8_t *>(info.ptr);
            self.no_channels = info.shape[0];
      ...
      .def_property(
          ...
          [](NDIlib_metadata_frame_t &self, const std::string &data) {
            static std::unordered_map<NDIlib_metadata_frame_t *, std::string>
                strs;
            strs[&self] = py::str(data);
            self.p_data = const_cast<char*>(strs[&self].c_str());
          });
  ...
  m.def(
      "find_destroy",
      [](py::capsule instance) {
        if (!instance || !instance.get_pointer()) {
          throw std::runtime_error("Invalid or already destroyed NDI finder");
        }
        auto p_instance =
  ...
  m.def(
      ...
      [](py::capsule instance) {
        if (!instance || !instance.get_pointer()) {
          throw std::runtime_error("Invalid or already destroyed NDI receiver");
        }
        auto p_instance =
  ...
  m.def(
      ...
      [](py::capsule instance, uint32_t timeout_in_ms) {
        if (!instance || !instance.get_pointer()) {
          throw std::runtime_error("Invalid NDI receiver instance");
        }
        if (std::string(instance.name()) != "RecvInstance") {
          throw std::runtime_error("Expected RecvInstance, got " + std::string(instance.name()));
        }
        auto p_instance =
            static_cast<NDIlib_recv_instance_type *>(instance.get_pointer());
        py::gil_scoped_release release;
  ...
  m.def(
      ...
      [](py::capsule instance) {
        if (!instance || !instance.get_pointer()) {
          throw std::runtime_error("Invalid or already destroyed NDI sender");
        }
        auto p_instance =
  ...
  py::class_<NDIlib_audio_frame_interleaved_16s_t>(m,
      ...
      .def_property(
          "data",
          [](py::object self_obj) {
            auto& self = self_obj.cast<NDIlib_audio_frame_interleaved_16s_t&>();
            size_t col = self.no_samples;
            size_t row = self.no_channels;
            ...
            std::vector<size_t> shape = {row, col};
            std::vector<size_t> strides = {col * size, size};
            // Keep the AudioFrameInterleaved16s object alive as long as the array exists
            return py::array(dtype, shape, strides, self.p_data, self_obj);
          },
  ...
  py::class_<NDIlib_audio_frame_interleaved_32s_t>(m,
      ...
      .def_property(
          "data",
          [](py::object self_obj) {
            auto& self = self_obj.cast<NDIlib_audio_frame_interleaved_32s_t&>();
            size_t col = self.no_samples;
            size_t row = self.no_channels;
            size_t size = sizeof(int32_t);
            auto dtype = py::dtype(py::format_descriptor<int32_t>::format());
            std::vector<size_t> shape = {row, col};
            std::vector<size_t> strides = {col * size, size};
            // Keep the AudioFrameInterleaved32s object alive as long as the array exists
            return py::array(dtype, shape, strides, self.p_data, self_obj);
          },
  ...
  m.def(
      ...
      [](py::capsule instance) {
        if (!instance || !instance.get_pointer()) {
          throw std::runtime_error("Invalid receiver instance");
        }
